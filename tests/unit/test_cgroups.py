...
def test_cgroups_available_true(tmp_path):
    ...
    from dpmd.cgroups import cgroups_available
    dpm_dir = tmp_path / "dpm"
    dpm_dir.mkdir()
    with patch("dpmd.cgroups.CGROUP_BASE", str(tmp_path)):
...
def test_cgroups_available_false_no_dir():
    ...
    from dpmd.cgroups import cgroups_available
    with patch("dpmd.cgroups.CGROUP_BASE", "/nonexistent/cgroup/path"):
...
def test_setup_cgroup_creates_dir_and_writes(tmp_path):
    ...
    from dpmd.cgroups import setup_cgroup
    with patch("dpmd.cgroups.CGROUP_BASE", str(tmp_path)):
...
def test_cleanup_cgroup_removes_dir(tmp_path):
    ...
    """
    from dpmd.cgroups import cleanup_cgroup
    cgroup_dir = tmp_path / "myproc"
    cgroup_dir.mkdir()
